...
export default function EventContext({ event, data }) {
  ...
  const handleChange = (event) => {
    ...
    event.target.checked === true &&
      dispatch(addContexts4User(username, data.context));
    event.target.checked === false &&
      dispatch(removeContexts4User(username, data.context));
  };
  ...
  return (
    <>
      <div className="event-title">
        <span style={{ color: "dodgerblue" }}>{event.user + " "}</span>
        has created a new context!
      </div>

      <div className="event-main">
        <span style={{ color: "tomato" }}>{" " + data.context}</span>
      </div>
      ...
      <div className="event-content">
        ...
        <Row className="highlights">
          <Col className="highlight-column">
            <div className="highlight-header">{subscribes}</div>

            <div className="highlight-title">Subscribers</div>
          </Col>

          <Col className="highlight-column">
            <div className="highlight-header">{scope}</div>

            <div className="highlight-title">Scope</div>
          </Col>

          <Col className="highlight-column">
            <div className="highlight-header">{rules_qty}</div>

            <div className="highlight-title">Rules</div>
          </Col>
        </Row>
        {/* <div className="scope">
        ...
        <div className="subscribtion">
          <div style={{ width: "25%" }}>
            <FormGroup>
              <FormControlLabel
                ...
                label={
                  <Typography variant="body3" color="textSecodary">
          ...
          >
            {reach ? (
              <span style={{ color: "dodgerblue" }}>Reached</span>
            ) : (
